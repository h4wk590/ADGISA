...
# Text boxes for converting, clearing, and decimal degrees
...
def convert():
    if not txtDegrees.get().isdigit():
        ...
        messagebox.showerror("Error", "Please use interger values to input data, not strings.")
            ...
            fltMinutes = float(txtMinutes.get()) / 60
            fltSeconds = float(txtSeconds.get()) / 3600
            result = txtDD + fltMinutes + fltSeconds
            txtDD.insert(0, result)
